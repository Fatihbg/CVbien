...
export const RegisterForm: React.FC<RegisterFormProps> = ({ onSuccess, onSwitchToLogin }) => {
  ...
  const handleEmailChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setTimeout(async () => {
      const emailExists = await verifyEmailExists(newEmail);
      setIsVerifyingEmail(false);
      
      if (!emailExists) {
        setEmailError('Cette adresse email n\'existe pas');
      } else {
        setEmailError('');
      }
    }, 1000);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // if (!emailExists) {
    //   alert('Cette adresse email n\'existe pas. Veuillez utiliser une adresse email valide.');
    //   setIsLoading(false);
    //   return;
    // }
    ...
    <div className="glass-card fade-in" style={{
      ...
      <div style={{ textAlign: 'center', marginBottom: '24px' }}>
        ...
        <p style={{
          ...
          Créez votre compte CVbien gratuit
        </p>
      </div>
